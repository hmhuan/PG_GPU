	...
	if (ix < width && iy < height)
	{
		...
		for (int filterR = 0; filterR < filterWidth; filterR++)
		{
			...
			{ 
				idx = iy + filterR - half;
				dy = ix + filterC - half;
				idx = min(height - 1, max(0, idx));
				dy = min(width - 1, max(0, dy));
				idx = idx * width + dy;
	
				val_filter = filter[filterR * filterWidth + filterC];
				G.x += val_filter * inPixels[idx].x; 
				G.y += val_filter * inPixels[idx].y;
				G.z += val_filter * inPixels[idx].z;
			}
		}
		outPixels[i].x = G.x;
		outPixels[i].y = G.y;
        ...
        uchar3 * outPixels)
{
	...
	int nRow = blockDim.y + filterWidth -  1;
	int nCol = blockDim.x + filterWidth -  1;
	for (int row = threadIdx.y; row < nRow; row += blockDim.y){
		for (int col = threadIdx.x; col < nCol; col += blockDim.x)
		{
			int inPixelsR = blockIdx.y * blockDim.y - filterWidth / 2 + row;
			int inPixelsC = blockIdx.x * blockDim.x - filterWidth / 2 + col;
			inPixelsR = min(height - 1, max(0, inPixelsR));
			inPixelsC = min(width - 1, max(0, inPixelsC));
			s_inPixels[row * nCol + col] = inPixels[inPixelsR * width + inPixelsC];
		}
	}
	__syncthreads();
	int iy =  blockIdx.y * blockDim.y + threadIdx.y;
	...
	if (ix < width && iy < height)
	{
		...
		for (int filterR = 0; filterR < filterWidth; filterR++)
		{
			for (int filterC = 0; filterC < filterWidth; filterC++)
			{
				dy = threadIdx.y + filterR;
				dx = threadIdx.x + filterC;
				idx = dy * nCol + dx;
				val_filter = filter[filterR * filterWidth + filterC];
				G.x += val_filter * s_inPixels[idx].x; 
				G.y += val_filter * s_inPixels[idx].y;
				G.z += val_filter * s_inPixels[idx].z;
			}
		}
		outPixels[i].x = G.x;
		outPixels[i].y = G.y;
	...
	if (ix < width && iy < height)
	{
		...
		for (int filterR = 0; filterR < filterWidth; filterR++)
		{
			for (int filterC = 0; filterC < filterWidth; filterC++)
			{
				...
				dx = threadIdx.x + filterC;
				idx = dy * nCol + dx;
				val_filter = dc_filter[filterR * filterWidth + filterC];
				G.x += val_filter * s_inPixels[idx].x; 
				G.y += val_filter * s_inPixels[idx].y;
				G.z += val_filter * s_inPixels[idx].z;
			}
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		else
		{
			...
			cudaMemcpyToSymbol(dc_filter, filter, FILTER_WIDTH * FILTER_WIDTH * sizeof(float));
		}
