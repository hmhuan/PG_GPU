...
// Chỗ này để sort by device do team viết sử dụng radix sort k = 1
        ...
        int useDevice=0, int * blockSizes=NULL)
{
    ...
    else if(useDevice == 2)
    {
        ...
        testSort(in, n, out, 1, blockSizes);
    }
    else // use device thrust
    {
    	printf("\nRadix sort by thrust device\n");
...
int main(int argc, char ** argv)
{
    ...
    // SORT BY MY DEVICE
    sort(in, n, out, nBits, 2, blockSizes);
    checkCorrectness(out, correctOut, n);
